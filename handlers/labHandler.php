    ...
    if(mysqli_num_rows($reportData)){
        while($reportDataRow=mysqli_fetch_array($reportData)){ 
            ...
            if($testname!=$reportDataRow[4])
            {
                ...
                $output.='<br><h4><u>'.$testname.'</u></h4><tbody>';
            }
            $output.='
            <tr>
...
function getPatLab(){
    ...
    if(mysqli_num_rows($repData)){
        ...
        $output.="<div class='row patientDataRow'>
            <div class='c-3 c-m-3 repId'>$row[0]</div>
            <div class='c-4 c-m-3 repDate'>$row[2]</div>
            <div class='c-4 c-m-5 repCmt' style='text-overflow: ellipsis;overflow: hidden;white-space: nowrap;'>$row[3]</div>
